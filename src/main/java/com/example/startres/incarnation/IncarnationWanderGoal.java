...
public class IncarnationWanderGoal extends Goal {
    ...
    public void start() {
        ...
        Vec3 randomPos = DefaultRandomPos.getPosTowards((PathfinderMob) mob, (int) wanderRange, 7, centerPos, 1.0); // 在中心位置周围随机选择一个位置
        if (randomPos == null) {

            randomPos = DefaultRandomPos.getPosTowards((PathfinderMob) mob, 5, 7, mob.position(), 1.0); // 如果找不到合适的位置，就在当前位置附近随机
        }
